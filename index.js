...
app.post("/Signup", async function (request, response) {
    ...
    if (userCheck) {
      ...
      const post = await client.db("day-43").collection("userdata").insertOne(da);
...
app.put("/Forgetpassword", async function (request, response) {
    ...
    const userCheck = await client.db("day-43").collection("userdata").findOne({ username: username });
      if(!userCheck){
        ...
        const composeMail = {
            ...
            const otpset = await client.db("day-43").collection("userdata").updateOne({ usertoken :userCheck.usertoken },{$set:{OTP : otp}});
        console.log(otpset)
...
app.post("/Profile/:usertoken", async function (request, response) {
    ...
    const userCheck = await client.db("day-43").collection("userdata").findOne({ username: request.params.usertoken });
